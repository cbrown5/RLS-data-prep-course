...
To use patchwork to combine our disparate plots, first we'll store our histogram and time-series plots as objects in R. We'll call them 'a' and 'b'.
...
Easy, wasn't it? If we'd rather have the two plots stacked on top of each other, we can do this instead.

```{r}
c <- a/b
```

Now lets add some plot labels and ggplot's 'theme_classic' to get the plots looking sleek. We'll use the function 'ggsave' to export the plot to the folder on your computer that is home to your R script.
